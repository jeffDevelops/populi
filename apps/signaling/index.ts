...
const server = Bun.serve({
  ...
  websocket: {
    open(ws: ServerWebSocket<WebSocketData>) {
      ...
      console.log(
        `Client ${clientId} joined room ${roomId}. Room size: ${room.size}`,
      )
    ...
    close(ws: ServerWebSocket<WebSocketData>) {
      ...
      for (const [roomId, room] of rooms.entries()) {
        for (const [clientId, client] of room.entries()) {
          if (client.ws === ws) {
            // Compare the WebSocket instances
            // Remove client from room
            room.delete(clientId)
            console.log(
              `Client ${clientId} left room ${roomId}. Room size: ${room.size}`,
            )
...
console.log(
  `Rift Signaling Server running at http://${server.hostname}:${server.port}`,
)
