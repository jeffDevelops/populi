...
vi.mock('child_process', async () => {
  ...
  return {
    ...
    execSync: vi.fn((command: string, options?: any) => {
      ...
      if (command.includes('powershell.exe') && command.includes('Run-Android-Emulator.ps1')) {
        return mockExecSync(command, options);
      }
      if (command.includes('powershell.exe') && command.includes('Launch-Swarm-Android.ps1')) {
        return mockExecSync(command, options);
      }
...
describe('Android Script Integration Tests', () => {
  ...
  describe('Single Instance Launch', () => {
    it('should execute single emulator launch script with correct parameters', async () => {
      ...
      const scriptPath = path.join(process.cwd(), 'scripts', 'Run-Android-Emulator.ps1');
    ...
    it('should launch iOS simulator instance on macOS', async () => {
      ...
      const scriptPath = path.join(process.cwd(), 'scripts', 'run-ios-simulator.sh');
      ...
      try {
        ...
        const simulators = execSync('xcrun simctl list devices | grep Booted', { encoding: 'utf8' });
  ...
  describe('Multi-Instance Launch', () => {
    it('should execute multi-instance launch script with correct parameters', async () => {
      ...
      const scriptPath = path.join(process.cwd(), 'scripts', 'Launch-Swarm-Android.ps1');
      ...
      const devices = execSync('adb devices', { encoding: 'utf8' });
      const emulatorCount = (devices.match(/emulator-\d+\s+device/g) || []).length;
    ...
    it('should handle insufficient emulators for multi-instance launch', async () => {
      ...
      mockExecSync.mockImplementation(() => {
        throw new Error('Insufficient emulators available for multi-instance launch');
      });
    ...
    it('should launch multiple iOS simulator instances on macOS', async () => {
      ...
      const scriptPath = path.join(process.cwd(), 'scripts', 'launch-multi-ios.sh');
  ...
  describe('Script Logic Verification', () => {
    ...
    it('should handle script execution with dry run mode', async () => {
      ...
      mockExecSync.mockReturnValue('DRY RUN: Would launch emulator with InstanceId=1');
  ...
  describe('Parameter Validation', () => {
    ...
    it('should validate emulator name parameter format', async () => {
      const validNames = ['Pixel_7_API_34', 'Galaxy_S24_API_34', 'Test_Emulator'];
  ...
  describe('Environment Validation', () => {
    it('should validate Android SDK commands are mocked correctly', () => {
      ...
      const emulatorResult = execSync('emulator -list-avds', { encoding: 'utf8' });
