...
export class MockXcrun {
  private availableSimulators: MockSimulator[] = [
    { name: 'iPhone 15', udid: 'ABC123', state: 'Shutdown', runtime: 'iOS 17.0' },
    { name: 'iPhone 14', udid: 'DEF456', state: 'Shutdown', runtime: 'iOS 17.0' },
    { name: 'iPad Pro', udid: 'GHI789', state: 'Shutdown', runtime: 'iOS 17.0' }
  ];
...
if [ "$1" = "-list-avds" ]; then
  ...
  const emulatorPath = path.join(binDir, isWindows ? 'emulator.bat' : 'emulator');
