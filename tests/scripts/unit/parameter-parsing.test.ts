  ...
  describe('PowerShell Scripts', () => {
    ...
    it('should validate InstanceId parameter', () => {
      const result = testPowerShellScript(
        ...
        '-InstanceId 5 -EmulatorName "Test"',
      )
    ...
    it('should handle invalid parameters gracefully', () => {
      ...
      expect(result.success === true || result.output.includes('DRY RUN')).toBe(
        true,
      )
    })
    ...
    it('should validate parameter types', () => {
      ...
      const result = testPowerShellScript(
        'Launch-Swarm-Android.ps1',
        '-NumberOfInstances "not-a-number"',
      )
  ...
  describe('Cross-Platform Parameter Consistency', () => {
    ...
    parameterTests.forEach(({ powershell, bash, expectedInOutput }) => {
      it(`should have equivalent behavior for ${expectedInOutput}`, () => {
        const psResult = testPowerShellScript(
          'Launch-Swarm-Android.ps1',
          powershell,
        )
        const bashResult = testBashScript('launch-swarm-android.sh', bash, {
          dryRun: true,
        })
        ...
        const bashContains =
          !bashOutput.includes('no such file') &&
          bashOutput.includes(expectedInOutput.toLowerCase())
